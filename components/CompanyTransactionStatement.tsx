...
export default function CompanyTransactionStatement({ operations, clients, vehicles, userRole, userIdentifier }: Props) {
  ...
  // --------------------------------------------------------------------------
  // 3. 데이터 필터링 (PC는 즉시 계산, 모바일은 조회 후 계산)
  // --------------------------------------------------------------------------
  const filteredData = useMemo(() => {
    // 화면 너비가 768px 미만(모바일)이고 조회를 안 눌렀으면 빈 배열 반환
    ...
    <div className="h-full flex flex-col md:flex-row bg-gray-100 overflow-hidden relative">
      ...
      <div className={`md:hidden flex-col h-full bg-gray-100 ${isSearched ? 'flex' : 'hidden'}`}>
        <div className="bg-white px-4 py-2 border-b flex justify-between items-center shrink-0 z-10 shadow-sm">
            <button onClick={() => setIsSearched(false)} className="flex items-center gap-1 text-slate-500 hover:text-slate-800 font-bold text-sm bg-slate-100 px-3 py-1.5 rounded-lg transition-colors"><ArrowLeft className="w-4 h-4" /> 다시 선택</button>
            <span className="text-xs font-bold text-slate-400">{startDate} ~ {endDate}</span>
        </div>
        <div className="flex-1 overflow-auto bg-gray-50 flex justify-center items-start pt-4 pb-24"><StatementContent /></div>
      ...
      <div className="hidden md:flex flex-1 h-full overflow-hidden">
        <div className="flex-1 overflow-auto bg-gray-50 flex justify-center items-start pt-8 pb-10"><StatementContent /></div>
        <div className="w-80 bg-white border-l p-6 shadow-xl z-20 flex flex-col">
            ...
            <div className="bg-gray-100 p-3 rounded-2xl mb-6"><label className="text-xs font-bold text-slate-500 mb-3 block uppercase tracking-wider">내역 구분</label><div className="flex gap-2"><button onClick={() => setViewType('SALES')} className={`flex-1 py-3 text-sm font-black rounded-xl transition-all shadow-sm ${viewType === 'SALES' ? 'bg-blue-600 text-white ring-2 ring-blue-200' : 'bg-white text-slate-400 hover:text-slate-600'}`}>매출 (청구)</button><button onClick={() => setViewType('PURCHASE')} className={`flex-1 py-3 text-sm font-black rounded-xl transition-all shadow-sm ${viewType === 'PURCHASE' ? 'bg-green-600 text-white ring-2 ring-green-200' : 'bg-white text-slate-400 hover:text-slate-600'}`}>매입 (지급)</button></div></div>
            <div className="space-y-6 flex-1">
                ...
                {!userRole?.includes('PARTNER') && (<div className="space-y-2"><label className="text-xs font-bold text-slate-500 uppercase tracking-wider">거래처 선택</label><select value={selectedClientName} onChange={(e) => setSelectedClientName(e.target.value)} className="w-full p-3 border border-slate-200 rounded-xl text-sm font-bold text-blue-600"><option value="전체">전체 거래처</option>{clients.map((c) => (<option key={c.id} value={c.clientName}>{c.clientName}</option>))}</select></div>)}
            </div>
