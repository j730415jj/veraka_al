                    ...
                    <div className="mt-4 pt-4 border-t border-slate-100">
                        {d.status === 'sent' && (
                            <div className="space-y-3">
                                ...
                                <button onClick={() => { if(!cardQuantities[d.id]&&!confirm('무게0?')) return; setActiveDispatchId(d.id); handleFinalSubmit(); }} className="w-full bg-slate-800 text-white py-3 rounded-xl font-bold text-sm">완료 전송</button>
                            </div>
                        )}
                        {d.status === 'completed' && (
